...
function SummaryCard({ icon: Icon, label, value, detail, tone = 'blue' }) {
  ...
  return (
    <article className="mission-panel min-w-0 p-4">
      <div className="flex min-w-0 items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="mission-muted text-sm font-medium">{label}</p>
          <p className={`mt-2 break-words font-display text-2xl font-semibold ${toneClass}`}>{value}</p>
          {detail ? <p className="mission-muted mt-1 break-words text-sm">{detail}</p> : null}
        </div>
        <span className="shrink-0 rounded-2xl bg-shalom-gold/40 p-3 text-shalom-deep dark:bg-white/10 dark:text-shalom-gold">
          <Icon size={22} />
        </span>
      </div>
    </article>
    ...
    <div className="mt-3 rounded-xl bg-shalom-cream/70 p-3 text-sm dark:bg-white/10">
      ...
      {items?.length ? (
        <ul className="mt-2 space-y-2">
          {items.map((item) => (
            <li key={item.id} className="flex min-w-0 flex-col gap-1 border-b border-line/60 pb-2 last:border-0 last:pb-0 dark:border-shalom-gold/10 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                <p className="break-words font-semibold">{item.item_name}</p>
      ...
      <section className="mission-panel min-w-0 p-4">
        <div className="grid min-w-0 grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          <div className="min-w-0">
            ...
            <p className="mission-muted break-words text-sm">{closing?.event ? `${closing.event.name} - ${formatDate(closing.event.event_date)}` : 'Geral do dia'}</p>
          </div>
          <div className="grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-[160px_minmax(190px,1fr)_auto] sm:items-end">
        ...
        <div className="mt-4 grid min-w-0 grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          <label className="block text-sm font-medium">
        ...
        <div className="mt-4 grid min-w-0 grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          {paymentMethods.length ? paymentMethods.map((item) => (
            <article key={`${item.payment_method}-${item.payment_status}`} className="mission-card min-w-0 p-4">
              <div className="flex min-w-0 items-start justify-between gap-3">
                <div className="min-w-0">
                ...
                <CreditCard size={20} className="shrink-0 text-shalom-orange dark:text-shalom-gold" />
              </div>
              <p className="mt-4 break-words font-display text-xl font-semibold text-shalom-blue dark:text-shalom-gold">{money.format(item.total)}</p>
      ...
      <section className="grid min-w-0 grid-cols-1 gap-5 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
        <div className="mission-panel min-w-0 p-4">
          <div className="flex min-w-0 items-center justify-between gap-3">
            ...
            <Clock3 size={22} className="shrink-0 text-shalom-orange dark:text-shalom-gold" />
          </div>
          ...
          <div className="mt-4 space-y-3">
            {loading ? (
              ...
              <article key={item.id} className="mission-card min-w-0 p-4">
                <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    <p className="break-words font-semibold">{item.customer_name}</p>
                ...
                {item.notes ? (
                  <div className="mt-3 rounded-xl bg-shalom-cream/70 p-3 text-sm dark:bg-white/10">
                    ...
                    <p className="mt-1 whitespace-pre-wrap break-words font-semibold">{item.notes}</p>
                  </div>
                ) : null}

                <SaleItemsList items={item.items} />

                <div className="mt-3 grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
                  <PaymentMethodSelect
        ...
        <div className="mission-panel min-w-0 p-4">
          <div className="flex min-w-0 items-center justify-between gap-3">
            ...
            <CalendarDays size={22} className="shrink-0 text-shalom-orange dark:text-shalom-gold" />
          </div>
          ...
          <div className="mt-4 space-y-3">
            {loading ? (
              ...
              <article key={sale.id} className="mission-card min-w-0 p-4">
                <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                ...
                <div className="mt-3 grid min-w-0 grid-cols-1 gap-3 text-sm sm:grid-cols-3">
                  <div className="rounded-xl bg-shalom-mist/70 p-3 dark:bg-white/10">
                ...
                {sale.customer_name || sale.notes ? (
                  <p className="mission-muted mt-3 break-words text-sm">

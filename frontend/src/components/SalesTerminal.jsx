...
function ComboCreatorModal({
  ...
  const selectedItems = products
    .map((product) => ({
      product,
      quantity: Number(
        draft.quantities[product.id] || 0
      )
    }))
    .filter((item) => item.quantity > 0)

  const regularPrice = selectedItems.reduce(
    (sum, item) =>
      sum +
  ...
  async function submit(event) {
    ...
    try {
      const combo = await api.createCombo({
        name: draft.name,
        sale_price: Number(
          String(draft.sale_price).replace(
            ',',
            '.'
          )
        ),
      ...
      >
        <div className="flex min-w-0 items-start justify-between gap-3 border-b border-line/80 p-4 dark:border-shalom-gold/10">
          <div className="min-w-0">
            ...
            >
              Criar combo ou promocao
            </h2>

            <p className="mission-muted mt-1 text-sm">
              Monte uma oferta usando os
          ...
          <button
            type="button"
            className="mission-btn shrink-0 border border-line/80 bg-white/70 p-2 dark:border-shalom-gold/10 dark:bg-white/10"
            onClick={onClose}
            aria-label="Fechar"
          >
            <X size={18} />
        ...
        <form
          className="dashboard-modal-body scrollbar-thin space-y-4 p-4"
          onSubmit={submit}
        >
          ...
          <div className="grid min-w-0 grid-cols-1 gap-4 sm:grid-cols-2">
            <label className="block text-sm font-medium">
              ...
              <input
                ...
                placeholder="Ex.: Queima de salgados"
                required
              />
            </label>
          ...
          <div>
            <div className="mb-2 flex min-w-0 flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
              ...
              <span className="mission-muted break-words text-sm">
                Preco normal:{' '}
                {money.format(regularPrice)}
              </span>
            </div>
              ...
              {products.map((product) => (
                ...
                >
                  <span className="min-w-0">
                    ...
                    <span className="mission-muted block break-words text-xs">
                      {money.format(
                        product.sale_price
                      )}{' '}
                      -{' '}
          ...
          {message ? (
            <p className="rounded-xl bg-shalom-wine/10 px-3 py-2 text-sm text-shalom-wine dark:text-rose-100">
              {message}
            </p>
          ) : null}
          ...
          <button
            type="submit"
            className="mission-btn mission-btn-primary flex w-full items-center justify-center gap-2 px-4 py-3 font-semibold"
            disabled={saving}
          >
            {draft.kind ===
            'promotion' ? (
...
export function SalesTerminal({
  ...
  function getCartLimitLabel(
    item
  ) {
    if (item.type === 'combo') {
      return `${item.stockLimit} disponiveis`
    }
  ...
  function addItem(
    ...
    if (stockLimit <= 0) {
      setMessage(
        `${item.name} esta indisponivel no estoque.`
      )
    ...
    if (
      ...
      setMessage(
        `${item.name} ja atingiu o limite disponivel (${
          type === 'combo'
            ? `${stockLimit} disponiveis`
            : formatQuantityWithUnit(
                stockLimit,
                item.unit
              )
  ...
  async function deleteCombo(
    ...
    if (
      !window.confirm(
        `Excluir ${comboType} "${combo.name}" do PDV?`
      )
    ) {
      return
    }
    ...
    try {
      ...
      setMessage(
        `${combo.is_promotion ? 'Promocao' : 'Combo'} ${combo.name} excluido do PDV.${
          result.sales_count
            ? ' Historico de vendas preservado.'
            : ''
        }`
      )
    } catch (err) {
  ...
  function validateSale() {
    ...
    if (exceededItem) {
      return `${exceededItem.name} passou do limite disponivel (${getCartLimitLabel(
        exceededItem
      )}).`
    }
  ...
  async function finishSale() {
    ...
    try {
      ...
      setMessage(
        `Venda #${sale.id} registrada: ${money.format(
          sale.total
        )}`
      )
      ...
      try {
        ...
        setMessage(
          `Venda #${sale.id} registrada: ${money.format(
            sale.total
          )}. Nao foi possivel atualizar a tela: ${refreshError.message}`
        )
      }
    } catch (err) {
      setConfirmationError(
  ...
  return (
    <div
      className={`min-w-0 ${
        cart.length
          ? 'flex h-full min-h-0 flex-col gap-3 xl:grid xl:h-auto xl:grid-cols-[minmax(0,1fr)_380px] xl:gap-5'
    ...
    >
      <div
        className={
          cart.length
            ? 'min-w-0 flex-1 space-y-5 overflow-y-auto pr-1 scrollbar-thin xl:contents xl:space-y-0 xl:overflow-visible xl:pr-0'
            : 'contents'
        }
      >
        <section className="min-w-0 space-y-5">
          {message ? (
            <p className="mission-panel px-4 py-3 text-sm text-shalom-deep dark:text-shalom-gold xl:hidden">
              {message}
            </p>
          ) : null}
          ...
          <div className="mission-panel min-w-0 p-4">
            <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button
                type="button"
                className="mission-btn mission-btn-gold flex min-h-11 w-full items-center justify-center gap-2 px-3 py-2 text-sm font-semibold sm:w-auto"
                onClick={() =>
                  setShowComboCreator(
                    true
                  )
            ...
            {combos.length ? (
                  ...
                  (combo) => {
                    ...
                    return (
                      <article
                        ...
                        className="mission-card min-w-0 p-4"
                      >
                        <div className="flex min-w-0 items-start justify-between gap-3">
                          ...
                          >
                            <span className="text-sm font-semibold uppercase tracking-[0.14em] text-shalom-orange dark:text-shalom-gold">
                              {combo.is_promotion
                                ? 'Promocao'
                                : 'Combo'}
                            </span>

                            <strong className="mt-1 block break-words font-display text-lg leading-snug">
                              {
                                combo.name
                              }
                            </strong>

                            <span className="mt-3 block text-xl font-semibold text-shalom-blue dark:text-shalom-gold">
                              {money.format(
                                combo.sale_price
                              )}
                            </span>
                            ...
                            {combo.savings >
                            0 ? (
                              <span className="mt-1 block text-xs font-semibold text-shalom-orange dark:text-shalom-gold">
                                Economize{' '}
                                {money.format(
                                  combo.savings
                                )}
              ...
              <div className="rounded-2xl border border-dashed border-shalom-gold/50 p-6 text-center mission-muted">
                Nenhum combo ativo.
              </div>
            )}
          </div>
          ...
          {Object.entries(
            ...
            ([
              ...
              <div
                key={category}
                className="mission-panel min-w-0 p-4"
              >
                      ...
                      return (
                        ...
                        >
                          <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                            <strong className="min-w-0 break-words font-display text-lg leading-snug">
                              {
                                product.name
                              }
                            </strong>
                          ...
                          <span className="mt-3 block text-2xl font-semibold text-shalom-blue dark:text-shalom-gold">
                            {money.format(
                              product.sale_price
                            )}
                          </span>
        ...
        <aside
          id="sale-checkout"
          className="mission-panel min-w-0 scroll-mt-24 p-4 xl:sticky xl:top-24 xl:self-start"
        >
          ...
          <div className="max-h-80 space-y-2 overflow-y-auto pr-1 scrollbar-thin">
            {cart.length ? (
              cart.map((item) => (
                <div
                  key={item.key}
                  className="mission-card min-w-0 p-3"
                >
          ...
          <label className="mt-4 block text-sm font-medium">
            ...
            >
              ...
              <option value="pagamento_pendente">
                Pagamento pendente
              </option>
            </select>
          </label>
          ...
          {message ? (
            <p className="mt-4 rounded-2xl bg-shalom-cream/70 px-3 py-2 text-sm text-shalom-deep dark:bg-white/10 dark:text-shalom-gold">
              {message}
            </p>
          ) : null}

          <div className="mt-5 flex items-center justify-between border-t border-shalom-gold/40 pt-4 dark:border-shalom-gold/10">
            <span className="mission-muted text-sm">
              Total
            </span>

            <strong className="font-display text-2xl text-shalom-blue dark:text-shalom-gold">
              {money.format(total)}
            </strong>
          </div>
      ...
      {cart.length ? (
        <div className="mobile-cart-summary shrink-0 xl:hidden">
          ...
          >
            ...
            <span className="hidden shrink-0 text-xs uppercase tracking-wide min-[360px]:inline">
              Finalizar
            </span>
          </button>
        </div>

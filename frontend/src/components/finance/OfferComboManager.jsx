...
export function OfferComboManager() {
  ...
  if (editor) {
    return (
      <ComboCreatorModal
        products={products}
        initialCombo={editor.combo || null}
        initialKind={editor.kind}
        onClose={() => setEditor(null)}
        onCreated={async (combo) => {
          setEditor(null)
          await loadData()
          setMessage(`${combo.name} salvo com sucesso.`)
        }}
      />
    )
  }
  ...
  return (
    <section className="min-w-0 px-3 py-4 sm:px-5 lg:px-0">
      ...
      <div className="mt-4 divide-y divide-line/80 border-y border-line/80 dark:divide-shalom-gold/10 dark:border-shalom-gold/10">

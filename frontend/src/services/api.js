const defaultApiUrl = typeof window !== 'undefined'
  ? `${window.location.protocol}//${window.location.hostname || 'localhost'}:4000/api`
  : 'http://localhost:4000/api'

const API_URL = import.meta.env.VITE_API_URL || defaultApiUrl

let authToken = localStorage.getItem('lanchonete_token') || ''

export function setToken(token) {
  authToken = token || ''
  if (token) localStorage.setItem('lanchonete_token', token)
  else localStorage.removeItem('lanchonete_token')
}

export function getToken() {
  return authToken
}

async function request(path, options = {}) {
  const headers = {
    ...(options.body instanceof FormData ? {} : { 'Content-Type': 'application/json' }),
    ...(authToken ? { Authorization: `Bearer ${authToken}` } : {}),
    ...options.headers
  }

  const response = await fetch(`${API_URL}${path}`, {
    ...options,
    headers,
    body: options.body && !(options.body instanceof FormData) ? JSON.stringify(options.body) : options.body
  })

  if (!response.ok) {
    const errorPayload = await response.json().catch(() => ({}))
    const error = new Error(errorPayload.message || 'Nao foi possivel completar a acao.')
    error.payload = errorPayload
    throw error
  }

  if (response.status === 204) return null
  return response.json()
}

async function download(path, filename) {
  const response = await fetch(`${API_URL}${path}`, {
    headers: authToken ? { Authorization: `Bearer ${authToken}` } : {}
  })

  if (!response.ok) throw new Error('Nao foi possivel baixar o arquivo.')

  const blob = await response.blob()
  const url = URL.createObjectURL(blob)
  const link = document.createElement('a')
  link.href = url
  link.download = filename
  link.click()
  URL.revokeObjectURL(url)
}

export const api = {
  login: (username, password) => request('/auth/login', { method: 'POST', body: { username, password } }),
  me: () => request('/auth/me'),
  changePassword: (payload) => request('/auth/change-password', { method: 'POST', body: payload }),
  users: () => request('/users'),
  createUser: (payload) => request('/users', { method: 'POST', body: payload }),
  resetUserPassword: (id) => request(`/users/${id}/reset-password`, { method: 'POST' }),
  deleteUser: (id) => request(`/users/${id}`, { method: 'DELETE' }),
  dashboard: () => request('/analytics/dashboard', { cache: 'no-store' }),
  publicLibraryCategories: () => request('/public/library/categories', { cache: 'no-store' }),
  publicLibraryProducts: (params = {}) => request(`/public/library/products?${new URLSearchParams(params)}`, { cache: 'no-store' }),
  publicLibraryProduct: (id) => request(`/public/library/products/${id}`, { cache: 'no-store' }),
  publicLibraryWhatsApp: (id) => request(`/public/library/products/${id}/whatsapp`, { cache: 'no-store' }),
  createPublicLibraryRequest: (payload) => request('/public/library/requests', { method: 'POST', body: payload }),
  publicLibraryRequest: (reference) => request(`/public/library/requests/${encodeURIComponent(reference)}`, { cache: 'no-store' }),
  libraryDashboard: (params = {}) => request(`/library/dashboard?${new URLSearchParams(params)}`, { cache: 'no-store' }),
  librarySpreadsheet: (params = {}) => request(`/library/spreadsheet?${new URLSearchParams(params)}`, { cache: 'no-store' }),
  librarySpreadsheetOptions: () => request('/library/spreadsheet/options', { cache: 'no-store' }),
  downloadLibrarySpreadsheet: (params = {}, format = 'csv') => download(`/library/spreadsheet/export?${new URLSearchParams({ ...params, format })}`, `livraria.${format}`),
  librarySellers: () => request('/library/sellers', { cache: 'no-store' }),
  librarySellersByVisibility: (visibility = 'active') => request(`/library/sellers?${new URLSearchParams({ visibility })}`, { cache: 'no-store' }),
  createLibrarySeller: (payload) => request('/library/sellers', { method: 'POST', body: payload }),
  updateLibrarySeller: (id, payload) => request(`/library/sellers/${id}`, { method: 'PATCH', body: payload }),
  removeLibrarySeller: (id) => request(`/library/sellers/${id}`, { method: 'DELETE' }),
  librarySellerMonitoring: () => request('/library/seller-monitoring', { cache: 'no-store' }),
  libraryRequests: (params = {}) => request(`/library/requests?${new URLSearchParams(params)}`, { cache: 'no-store' }),
  libraryRequest: (reference) => request(`/library/requests/${encodeURIComponent(reference)}`, { cache: 'no-store' }),
  updateLibraryRequestItems: (reference, payload) => request(`/library/requests/${encodeURIComponent(reference)}/items`, { method: 'PATCH', body: payload }),
  cancelLibraryRequest: (reference) => request(`/library/requests/${encodeURIComponent(reference)}/cancel`, { method: 'PATCH' }),
  reassignLibraryRequest: (reference, payload) => request(`/library/requests/${encodeURIComponent(reference)}/reassign`, { method: 'PATCH', body: payload }),
  convertLibraryRequest: (reference, payload) => request(`/library/requests/${encodeURIComponent(reference)}/convert`, { method: 'POST', body: payload }),
  libraryCategories: () => request('/library/categories', { cache: 'no-store' }),
  createLibraryCategory: (payload) => request('/library/categories', { method: 'POST', body: payload }),
  libraryProducts: (params = {}) => request(`/library/products?${new URLSearchParams(params)}`, { cache: 'no-store' }),
  createLibraryProduct: (payload) => request('/library/products', { method: 'POST', body: payload }),
  updateLibraryProduct: (id, payload) => request(`/library/products/${id}`, { method: 'PATCH', body: payload }),
  createLibraryMovement: (payload) => request('/library/movements', { method: 'POST', body: payload }),
  libraryMovements: () => request('/library/movements?limit=120', { cache: 'no-store' }),
  createLibrarySale: (payload) => request('/library/sales', { method: 'POST', body: payload }),
  librarySales: () => request('/library/sales?limit=120', { cache: 'no-store' }),
  products: (params = {}) => request(`/products?${new URLSearchParams(params)}`),
  productCategories: () => request('/products/categories'),
  createProduct: (product) => request('/products', { method: 'POST', body: product }),
  importProducts: (payload) => request('/products/import', { method: 'POST', body: payload }),
  productHistory: (id) => request(`/products/${id}/history`, { cache: 'no-store' }),
  updateProduct: (id, product) => request(`/products/${id}`, { method: 'PATCH', body: product }),
  deleteProduct: (id, payload) => request(`/products/${id}`, { method: 'DELETE', body: payload }),
  combos: () => request('/combos'),
  createCombo: (combo) => request('/combos', { method: 'POST', body: combo }),
  deleteCombo: (id) => request(`/combos/${id}`, { method: 'DELETE' }),
  createSale: (sale) => request('/sales', { method: 'POST', body: sale }),
  pendingPayments: () => request('/sales/pending', { cache: 'no-store' }),
  cashClosing: (params = {}) => request(`/sales/closing?${new URLSearchParams(params)}`, { cache: 'no-store' }),
  saveCashClosing: (payload) => request('/sales/closing', { method: 'POST', body: payload }),
  confirmSalePayment: (id, payload) => request(`/sales/${id}/payment`, { method: 'PATCH', body: payload }),
  events: () => request('/sales/events'),
  createEvent: (event) => request('/sales/events', { method: 'POST', body: event }),
  updateEvent: (id, event) => request(`/sales/events/${id}`, { method: 'PATCH', body: event }),
  sales: () => request('/sales?limit=80'),
  movements: () => request('/inventory/movements'),
  batches: (params = {}) => request(`/inventory/batches?${new URLSearchParams(params)}`),
  updateBatch: (id, payload) => request(`/inventory/batches/${id}`, { method: 'PATCH', body: payload }),
  productStock: (id, params = {}) => request(`/inventory/products/${id}/stock?${new URLSearchParams(params)}`),
  createMovement: (movement) => request('/inventory/movements', { method: 'POST', body: movement }),
  postEventInventories: () => request('/inventory/post-event'),
  postEventInventory: (id) => request(`/inventory/post-event/${id}`),
  createPostEventInventory: (inventory) => request('/inventory/post-event', { method: 'POST', body: inventory }),
  sheets: () => request('/spreadsheet/sheets'),
  sheet: (sheet) => request(`/spreadsheet/${sheet}`),
  updateSheetProduct: (id, payload) => request(`/spreadsheet/produtos/${id}`, { method: 'PATCH', body: payload }),
  updateSheetSale: (id, payload) => request(`/spreadsheet/vendas/${id}`, { method: 'PATCH', body: payload }),
  deleteSheetSale: (id, payload) => request(`/spreadsheet/vendas/${id}`, { method: 'DELETE', body: payload }),
  confirmSheetSalePayment: (id, payload) => request(`/spreadsheet/vendas/${id}`, { method: 'PATCH', body: payload }),
  updateSheetMovement: (id, payload) => request(`/spreadsheet/movimentacoes/${id}`, { method: 'PATCH', body: payload }),
  setupStatus: () => request('/system/setup-status', { cache: 'no-store' }),
  updateInitialLoad: (payload) => request('/system/initial-load', { method: 'PATCH', body: payload }),
  systemHealth: () => request('/system/health', { cache: 'no-store' }),
  auditLogs: (params = {}) => request(`/system/audit-logs?${new URLSearchParams(params)}`, { cache: 'no-store' }),
  telegramAlertsStatus: () => request('/system/telegram-alerts', { cache: 'no-store' }),
  updateTelegramAlerts: (payload) => request('/system/telegram-alerts', { method: 'PATCH', body: payload }),
  testTelegramAlerts: () => request('/system/telegram-alerts/test', { method: 'POST' }),
  resetOperationalData: (payload) => request('/system/reset-operational-data', { method: 'POST', body: payload }),
  backup: () => request('/backup', { method: 'POST' }),
  backups: () => request('/backup'),
  restoreBackup: (file, payload) => request(`/backup/${encodeURIComponent(file)}/restore`, { method: 'POST', body: payload }),
  downloadReport: (type, format) => download(`/reports/export?type=${type}&format=${format}`, `${type}.${format}`)
}
